<script lang="ts" context="module">
	declare let window : WindowAPI;
</script>

<script lang="ts">
	import Card, { hasMaximizedCard } from '../components/Card.svelte';
	import CardPlaceholder from '../components/CardPlaceholder.svelte';
	import CascadingImages from '../components/CascadingImages.svelte';
</script>

<!-- <div class="absolute bg-main-color flex flex-col flex-wrap content-center justify-center text-white w-screen h-screen z-50">
	<h1 class="text-white text-4xl font-main">
		<span class="text-white animate-pulse">anima</span>
	</h1>
	<div class="flex justify-center mt-5 animate-spin">
		<i class="fas fa-circle-notch text-4xl"></i>
	</div>
</div> -->

<div>
	<CascadingImages/>
	<div class="flex flex-col space-y-4 px-5 mt-10">
		<div>
			<div class="text-white text-left font-main text-2xl my-2">
				Popular <i class="fas fa-fire-alt text-orange"></i>
			</div>
			{#await window.api.search({name: ''})}
				<div class="text-white content-start w-full h-60">
					
				</div>
			{:then populars}
				<div class="sc flex {$hasMaximizedCard ? 'space-x-0' : 'space-x-4'} p-1 content-start overflow-x-scroll">
					{#each populars as game (game.id)}
						{#await window.api.compress(game.background_image)}
							<CardPlaceholder/>
						{:then image}
							<Card gameID={game.id} title={game.name} image={image}/>
						{/await}
					{/each}
				</div>
			{/await}
		</div>
	
		<div>
			<div class="text-white text-left font-main text-2xl my-2">
				Big Releases <i class="fas fa-meteor text-yellow-400"></i>
			</div>
			{#await window.api.search({name: '', date: '2021-01-01,2021-12-31'})}
				<div class="text-white content-start h-60">
					
				</div>
			{:then populars}
				<div class="sc flex {$hasMaximizedCard ? 'space-x-0' : 'space-x-4'} content-start overflow-x-scroll">
					{#each populars as game (game.id)}
						{#await window.api.compress(game.background_image)}
							<CardPlaceholder/>
						{:then image}
							<Card gameID={game.id} title={game.name} image={image}/>
						{/await}
					{/each}
				</div>
			{/await}
		</div>
	</div>
</div>