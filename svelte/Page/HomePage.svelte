...
<div class:hidden={!isReady}>
	<CascadingImages ready={isReady}/>
	<div class="flex flex-col space-y-4 px-5 mt-10">
		<div>
			...
			<div class="sc flex {$hasMaximizedCard ? 'space-x-0' : 'space-x-4'} p-1 content-start overflow-x-scroll">
				{#await window.api.compressFiles(popular.map(el => el.background_image)) then images}
					{#each popular as game, i (game.id)}
						<Card gameID={game.id} title={game.name} image={images[i]}/>
					{/each}
				{/await}
			</div>
		</div>
		...
		<div>
			...
			<div class="sc flex {$hasMaximizedCard ? 'space-x-0' : 'space-x-4'} content-start overflow-x-scroll">
				{#await window.api.compressFiles(releases.map(el => el.background_image)) then images}
